...
public class MainImport3 {
    private static final Logger logger = LoggerFactory.getLogger(MainImport3.class);
    ...
    public static void importDataToSQLFile() throws Exception {
        ...
        if (file2.exists()) {
            deleteFile(file2);
            logger.info("删除GOODS成功");
        }
        File file3 = new File("D:\\UnicomStore.sql");
        if (file3.exists()) {
            deleteFile(file3);
            logger.info("删除UnicomStore.sql成功");
        }

        OutputStream outputStream = new FileOutputStream("D:\\sqlimport\\unicom\\UnicomStore.sql");
        OutputStream out_item = new FileOutputStream("D:\\sqlimport\\unicom\\item.sql");
        OutputStream out_brand = new FileOutputStream("D:\\sqlimport\\unicom\\brand.sql");
        OutputStream out_attach_file = new FileOutputStream("D:\\sqlimport\\unicom\\attach_file.sql");
        OutputStream out_type = new FileOutputStream("D:\\sqlimport\\unicom\\type.sql");
        OutputStream out_sku = new FileOutputStream("D:\\sqlimport\\unicom\\sku.sql");
        OutputStream out_type_item_relation = new FileOutputStream("D:\\sqlimport\\unicom\\type_item_relation.sql");
        OutputStream out_good_prop = new FileOutputStream("D:\\sqlimport\\unicom\\good_prop.sql");
        //加载xlsx文件
        InputStream inputStream = new FileInputStream("D:\\工作数据\\工作数据\\联通积分商城\\prod.xlsx");
...
//                    }
                ...
                t_item.setSort(excel_item.getId()+11000); //todo 排序 这个排序 是每次最需要注意的地方
                ...
                if (excel_item.getPic() != null) {
                    picurl = anlsImgGoods(brand.getBrand_no(), t_item.getItem_no(), excel_item.getPic(),
                            outputStream, "t_item", t_item.getItem_id(),out_attach_file);
                }
                ...
                good_prop.setPropName("giftid");
                Pattern pattern = Pattern.compile("(?<=http://jf\\.10010\\.com/gift/)[A-Z0-9]*(?=\\.html)");
                Matcher matcher = pattern.matcher(excel_item.getLink());
                if (matcher.find()) {
                    good_prop.setValue(matcher.group());
                } else {
                    logger.error("未获取到商品的wareid");
                    System.exit(0);
                }
...
//                }
    ...
    private static Excel_item_china_mobile getRowData(Row row) {
        ...
        if(cellLimitedNum != null && cellLimitedNum.getCellType() == Cell.CELL_TYPE_STRING)
        {
            if("无".equals(cellLimitedNum.getStringCellValue()))
            {
                item.setLimitedNum(0);      //0: 无限制
            }
            else
            {
                logger.error("限制信息出错,{}",cellLimitedNum.getStringCellValue());
                System.exit(0);
            }
...
//        sb.append("DATE_FORMAT(" + sdf.format(t_stock.getEffective_time()) + ",%Y-%m-%d)" + ",");
    ...
    private static String anlsImgGoods(String brandCode, String productCode,
                                       String imgtype, OutputStream out, String tabname, Long productid,
                                       OutputStream out_detail
                                        ) throws IOException {
        String path = "D:\\工作数据\\工作数据\\联通积分商城\\img\\";
        String returl = null;
        Integer cnt = 1;
//        while(true) {
        ...
        if (file.exists()) {
            ...
            out_detail.write(sql2.getBytes());
        } else {
